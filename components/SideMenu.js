...
function SideMenu() {
  ...
  const handleScroll = () => {
    ...
    for (let i = sections.length - 1; i > 0; i--) {
      ...
      if (section) {
        if (section.offsetTop <= scrollPosition + 400) {
    ...
    >
      <div className="flex w-full flex-1 flex-col gap-10 justify-between items-center">
        ...
        <div className="flex flex-col justify-center items-center gap-2">
          <Image
            src={"/assets/mountains/logo.png"}
            width={100}
            height={100}
            alt="AlpineHunter Image"
            className="z-10 rounded-full"
          />
          <p className="font-bold text-black/75">AlpineHunter</p>
        </div>
        {/* 中サイトメニュー */}
        <div>
        ...
        <div className="flex flex-col items-center justify-center text-center">
          <div className="flex flex-col lg:flex-row items-center gap-4 mb-3">
            <a
              href="https://www.instagram.com/laughing.man0118?igsh=bnVrMWpyb25kcWVu"
              target="_blank"
          ...
          <p className="max-lg:hidden ">©2024AlpineHunter</p>
        </div>
      </div>
    </section>
  );
